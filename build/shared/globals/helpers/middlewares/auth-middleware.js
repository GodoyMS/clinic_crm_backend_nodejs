...
class AuthMiddleware {
    verifyUser(req, _res, next) {
        ...
        try {
            ...
            throw new notAuthorizedError_1.NotAuthorizedError('Token is invalid. Please login again.');
        }
        next();
    }
    checkAuthentication(req, _res, next) {
        if (!req.currentUser) {
            throw new notAuthorizedError_1.NotAuthorizedError('Authentication is required to access this route.');
        }
        next();
    }
}
