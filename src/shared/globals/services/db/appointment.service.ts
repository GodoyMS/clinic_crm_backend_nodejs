...
class AppointmentService {
  ...
  public async deleteAppointmentByID(id:string):Promise<IAppointmentDocument>{
   const deletedAppointment:IAppointmentDocument= (await AppointmentModel.findByIdAndDelete(id).exec()) as IAppointmentDocument;
  ...
  public async getAppointmentById(id:string):Promise<IAppointmentDocument>{
   const appointment:IAppointmentDocument= (await AppointmentModel.findById(id).populate('doctor').populate('patient').exec()) as IAppointmentDocument;
   return appointment;
  }
